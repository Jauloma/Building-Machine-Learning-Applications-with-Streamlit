...
try:
    ...
    with st.form("This form", clear_on_submit=True):
        ...
        frequency = st.text_input("Frequency 'D' for Daily 'W' for weekly ").upper() #convert to string and change to upper

        Number_of_days = st.number_input("Number of day(s)/Week(s)") # don't convert to int yet
    ...
    if submit:
        ...
        if frequency == "D" or frequency == 'W':
            ...
            # convert Number_of_days to int
            Number_of_days = int(Number_of_days)

            # pass inputs to the model (To make predictions, prophet requires number of days and frequency)
            future = model.make_future_dataframe(periods=Number_of_days, freq=frequency, include_history=False)

            # Make prediction
            forecast = model.predict(future)

            # show results
            st.write('[INFO]: The whole results')
            st.write(forecast)
    ...
    if option1:
        ...
        st.sidebar.write('This is a time series analysis & forecasting problem. In this project, we shall predict store sales on data from Corporation Favorita, a large Ecuadorian-based grocery retailer. Specifically, this app predicts the sales for up to weeks in advance for corporation favorita ')
        
except:
    st.error('Something went wrong: Make sure you entered the correct number of days, otherwise contact admin!')
